...
public class EmailGroupRepositoryIT extends AbstractIntegrationTest {
    ...
    public void testCreateUpdateDeleteProfile() {
        try {
            ...
            Profile profileCreated = groupRepository.createProfile(createProfile(profileId));
            assertNotNull(profileCreated);
            assertEquals(profileCreated, groupRepository.getProfile(createProfileKey(profileId)));
            ...
            Profile profileUpdated = groupRepository.getProfile(createProfileKey(profileId));
            assertEquals(profileCreated, profileUpdated);
            assertEquals(profileCreated.getFirstName(), profileUpdated.getFirstName());
            ...
            assertEquals(profileCreated.getMobilePhone(), profileUpdated.getMobilePhone());
            assertEquals(profileCreated.getCountryCode(), profileUpdated.getCountryCode());
            assertEquals(profileCreated.getComment(), profileUpdated.getComment());
            assertTrue(groupRepository.deleteProfile(profileCreated.getProfileKey()));
    ...
    public void testCreateUpdateDeleteGroup() {
        try {
            ...
            assertEquals(groupCreated.getDescription(), groupUpdated.getDescription());
            assertEquals(groupCreated.getIncludeVotes(), groupUpdated.getIncludeVotes());
            assertEquals(groupCreated.getExcludeVotes(), groupUpdated.getExcludeVotes());
    ...
    @Test
    public void testAddProfileToGroup() {
        try {
            GroupRepository groupRepository = new EmailGroupRepository(connector);
            groupRepository.createGroup(group1);
            assertFalse(groupRepository.getGroupProfileKeys(group1.getGroupKey()).contains(profile1.getProfileKey()));
            groupRepository.addProfileToGroup(group1.getGroupKey(), profile1.getProfileKey());
            assertTrue(groupRepository.getGroupProfileKeys(group1.getGroupKey()).contains(profile1.getProfileKey()));
            groupRepository.removeProfileFromGroup(group1.getGroupKey(), profile1.getProfileKey());
            assertFalse(groupRepository.getGroupProfileKeys(group1.getGroupKey()).contains(profile1.getProfileKey()));
        } catch(RepositoryException ex) {
            ex.printStackTrace();
            fail();
    ...
    @Test
    public void testCreateMyProfile() {
        try {
            GroupRepository groupRepository = new EmailGroupRepository(connector);
